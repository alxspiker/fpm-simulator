...
Render the Diagnostic HUD videos:

```powershell
python .\render_mp4.py --mode all
```
...
The toy lattice run uses `8 x 8 x 4 = 256` daemons. The current long run leaves most daemons active while a small exhausted fraction halts natively.

| Quantity | Value |
| --- | ---: |
| Active daemons | `222 / 256` |
| Halted daemons | `34 / 256` |
...
| points | `12` |
| lattice | `32 x 32 x 4` |
| fitted scale | `237.217010553100` |
| RMSE | `10.3271543310221 km/s` |
| normalized RMSE | `0.270462356988795` |

The returned C++ raw substrate velocities are finite, monotone-ish, and derived from route-cost measurements:

```text
0.155069, 0.158457, 0.160695, 0.161795, 0.161281, 0.164131,
0.160553, 0.163067, 0.165868, 0.160874, 0.164165, 0.162846
```
...
## Diagnostic HUD Video Series

The renderer now produces a small AxCore video suite from the JSON telemetry. The older `artifacts/causal_wave_render.gif` was removed because it was generated by the previous single-purpose renderer and no longer represented the canonical HUD output. The fresh MP4 set below is regenerated from the current multi-mode renderer.
...
```powershell
python .\render_mp4.py --mode photon
python .\render_mp4.py --mode torsion
python .\render_mp4.py --mode gravity
python .\render_mp4.py --mode time-dilation
```

The script auto-discovers the winget `Gyan.FFmpeg` install when `ffmpeg.exe` is not on PATH, and falls back to GIF only if FFmpeg is truly unavailable.

### Causal Wave / Emergent Photon

<video src="artifacts/causal_wave_render.mp4" controls muted loop></video>

[Open MP4](artifacts/causal_wave_render.mp4)

Visualizes the discrete `Z^3` memory registers, the cyan causal wavefront, and the crimson `E=0` ZOMBIE wake. This is the propagation proof: one spatial cell per universal tick.

### Dynamic Torsion / ZOMBIE Snap

<video src="artifacts/torsion_snap_render.mp4" controls muted loop></video>

[Open MP4](artifacts/torsion_snap_render.mp4)

Shows Node A and Node B connected by the pure-gauge torsion link while global energy drains. The CHSH meter climbs from the classical bound (`2.0`) to the Tsirelson limit (`2.828427`) as the substrate enters ZOMBIE mode.

### Baryonic Cluster / Emergent Gravity Well

<video src="artifacts/gravity_well_render.mp4" controls muted loop></video>

[Open MP4](artifacts/gravity_well_render.mp4)

Animates the shell-by-shell route-cost measurement around the central baryonic load, with the emergent rotation curve building in the HUD.

### Time-Dilation Gradient

<video src="artifacts/time_dilation_render.mp4" controls muted loop></video>

[Open MP4](artifacts/time_dilation_render.mp4)

Shows near-cluster and far-vacuum tick counters side by side so the execution-rate gradient is visible as clock drift.
...
|   |-- causal_wave_render.mp4
|   |-- torsion_snap_render.mp4
|   |-- gravity_well_render.mp4
|   |-- time_dilation_render.mp4
|   `-- *.png
|-- compile_and_run.ps1
|-- fpm_axcore_plots.py
|-- render_mp4.py
